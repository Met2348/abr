...
CURRENT_STAGE="init"
# 这个脚本是“跨任务验证入口”，核心只改 ACTIVE_CROSS_TASK_GROUP 与 RUN_PREFIX。
...
resolve_group() {
  ...
  # 新增跨任务组时，务必确认 SOURCE_RUN_NAME_PREFIX 指向已完成训练的 run 前缀。
...
run_eval_spec() {
  ...
  # pre=base 模型；post=source adapter。两者共用完全相同的评测输入。
...
# 允许手动指定 SOURCE_PHASE_B_RUN_DIR；不指定时自动取“同前缀最新目录”。
