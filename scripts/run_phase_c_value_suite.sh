...
resolve_group() {
  ...
  case "$ACTIVE_PHASE_C_GROUP" in
    ...
    C2_STRATEGYQA_CQR_SMOKE)
      GROUP_TITLE="Phase C2 StrategyQA CQR Smoke (CQR1-4)"
      GROUP_INTENTION="Validate CQR corruption repair (semantic + balanced generation) and stratified C2 sampling on a fast subset."
      GROUP_OBSERVE="Track corruption-type mix, pair-quality metrics, and C2 calibration/ranking movement under repaired supervision."
      GROUP_EXPECT="Compared with legacy quality-first smoke, CQR should improve pair margin quality and ranking stability."
      GROUP_DATASET="strategyqa"
      TRAIN_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/train.jsonl"
      EVAL_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/validation.jsonl"
      TRAIN_MAX_SAMPLES="${TRAIN_MAX_SAMPLES:-256}"
    ...
    C2_STRATEGYQA_CQR_FULL)
      GROUP_TITLE="Phase C2 StrategyQA CQR Full (CQR1-4)"
      GROUP_INTENTION="Run the full StrategyQA CQR recipe with repaired corruption generation and stratified C2 training."
      GROUP_OBSERVE="Assess whether CQR improvements hold at full train/validation scale."
      GROUP_EXPECT="If CQR is effective, full-run pair diagnostics and ranking metrics should improve over legacy quality-first full."
      GROUP_DATASET="strategyqa"
      TRAIN_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/train.jsonl"
      EVAL_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/validation.jsonl"
      TRAIN_MAX_SAMPLES="${TRAIN_MAX_SAMPLES:-}"
      ...
      C2_EPOCHS="${C2_EPOCHS:-10}"
      C2_USE_CONTRASTIVE="${C2_USE_CONTRASTIVE:-1}"
      C1_PREP_EXTRA_ARGS_DEFAULT="${C1_PREP_EXTRA_ARGS_DEFAULT:---corruption-selection-policy cqr_balanced --max-corruptions-per-prefix 4 --min-non-step-drop-per-prefix 1 --max-step-drop-per-prefix 1 --enable-negation-flip --enable-comparator-flip --enable-condition-reversal --enable-entity-substitution --build-pair-quality --pair-rollout-count 16 --target-alpha 1.0 --target-beta 1.0 --target-ci-z 1.96 --target-weight-floor 0.1 --target-weight-gamma 1.0 --pair-delta-q-min 0.20 --pair-z-min 0.5 --rollout-two-stage --rollout-stage1-count 8 --rollout-stage2-count 24 --rollout-uncertain-band 0.2 --rollout-uncertain-ci-width 0.3}"
      ...
      C2_EVAL_EXTRA_ARGS_DEFAULT="${C2_EVAL_EXTRA_ARGS_DEFAULT:---posthoc-calibration from_run}"
      ;;
    C2_STRATEGYQA_CQR_RERUN_TRICK10)
      GROUP_TITLE="Phase C2 StrategyQA CQR Rerun of Trick-10 Baseline"
      GROUP_INTENTION="Re-run prior Trick-10 recipe on CQR-repaired C1 artifacts to isolate data-quality effect."
      GROUP_OBSERVE="Compare against historical Trick-10 outputs under the same C2 objective family."
      GROUP_EXPECT="If old degradation was driven by corruption design, CQR rerun should improve pair ordering with similar calibration settings."
      GROUP_DATASET="strategyqa"
      TRAIN_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/train.jsonl"
      EVAL_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/validation.jsonl"
      TRAIN_MAX_SAMPLES="${TRAIN_MAX_SAMPLES:-256}"
      ...
      C2_EVAL_EXTRA_ARGS_DEFAULT="${C2_EVAL_EXTRA_ARGS_DEFAULT:---posthoc-calibration from_run}"
      ;;
    C2_STRATEGYQA_CQR_RERUN_QUALITY_FIRST)
      GROUP_TITLE="Phase C2 StrategyQA CQR Rerun of Quality-First Baseline"
      GROUP_INTENTION="Re-run old quality-first recipe with CQR C1 generation while keeping C2 close to historical settings."
      GROUP_OBSERVE="Check if CQR data quality alone recovers ranking signal without major C2 objective changes."
      GROUP_EXPECT="Improved pair metrics with modest calibration impact relative to old quality-first smoke."
      GROUP_DATASET="strategyqa"
      TRAIN_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/train.jsonl"
      EVAL_INPUT_JSONL="assets/artifacts/phase_a_prepared/strategyqa/16f7dd639f3e/validation.jsonl"
      TRAIN_MAX_SAMPLES="${TRAIN_MAX_SAMPLES:-256}"
    ...
    *)
      ...
      echo "Supported groups: C2_STRATEGYQA_SMOKE, C2_STRATEGYQA_FULL, C2_STRATEGYQA_TRICK1_BCE, C2_STRATEGYQA_TRICK2_POSTHOC_TEMP, C2_STRATEGYQA_TRICK3_ADAPTIVE_BALANCE, C2_STRATEGYQA_TRICK4_ISOTONIC, C2_STRATEGYQA_TRICK5_WEIGHTED_CAL, C2_STRATEGYQA_TRICK6_PAIR_FILTER, C2_STRATEGYQA_TRICK7_COMBINED, C2_STRATEGYQA_TRICK8_LABEL_SMOOTH, C2_STRATEGYQA_TRICK9_HARD_NEG_MINING, C2_STRATEGYQA_TRICK10_K16_COMBINED, C2_STRATEGYQA_QUALITY_FIRST, C2_STRATEGYQA_QUALITY_FIRST_FULL, C2_STRATEGYQA_CQR_SMOKE, C2_STRATEGYQA_CQR_FULL, C2_STRATEGYQA_CQR_RERUN_TRICK10, C2_STRATEGYQA_CQR_RERUN_QUALITY_FIRST"
      exit 1
      ;;
  esac
}
...
# 统一从 standalone eval 抽关键指标，避免混入训练期临时值。
