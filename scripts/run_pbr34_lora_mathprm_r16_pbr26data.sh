#!/usr/bin/env bash
# run_pbr34_lora_mathprm_r16_pbr26data.sh
#
# 用途 / Purpose:
#   PBR34: Math-PRM-7B + LoRA r=16 top-4 layers + PBR26 data (7366 pairs)
#   Tests the RANK effect: PBR33 uses r=8; PBR34 doubles LoRA rank to r=16 with the same data.
#   Hypothesis: higher LoRA rank (720K vs 360K params) + richer data → potential new SOTA.
#
# Compare with:
#   PBR31: Math-PRM-7B + LoRA r=8 top-4 + PBR12 data (5705 pairs)
#   PBR33: Math-PRM-7B + LoRA r=8 top-4 + PBR26 data (7366 pairs) [data ablation vs PBR31]
#   PBR34: Math-PRM-7B + LoRA r=16 top-4 + PBR26 data (7366 pairs) [rank ablation vs PBR33]
#
...
CUDA_VISIBLE_DEVICES="${CUDA_DEVICE}" "${PY}" -u scripts/phase_e_train_value_lora.py \
    ...
    --run-name phase_e_pbr34_lora_mathprm_r16_top4_pbr26data_s42 \
...
CUDA_VISIBLE_DEVICES="${CUDA_DEVICE}" "${PY}" -u scripts/phase_e_eval_benchmark.py \
    ...
    --require-cuda \
    2>&1 | tee -a "${LOGDIR}/pbr34_lora_mathprm_r16_top4_pbr26data.log"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] PBR34 complete."
