...
CURRENT_STAGE="init"
# 通常只需要改 ACTIVE_PHASE_B_GROUP 和 RUN_PREFIX；其它建议先保持默认。
...
resolve_group() {
  ...
  # 新增 B 组时，优先复制一个最接近的分组并只改 CONFIG_JSON/EVAL_SPECS。
...
run_eval_spec() {
  ...
  # pre 阶段评估 frozen base，post 阶段评估训练后 adapter；便于直接算增益。
...
if [[ "$AUTO_GAIN_EVAL" -eq 1 ]]; then
  # 训练前先跑同一套评估，形成可对比的 before metrics。
...
if [[ "$AUTO_GAIN_EVAL" -eq 1 ]]; then
  ...
  # 训练后复跑同一套评估；若这里改了输入或 token，上下对比会失真。
  while IFS='|' read -r label input_jsonl decode_mode max_new_tokens; do
    [[ -z "$label" ]] && continue
